...
void main() {
  ...
  // Apply simplex noise animation to the coin
  shapeUV *= .002;
  ...
  // Sample the coin texture
  vec4 texColor = texture2D(u_texture, vUv);
  
  // Apply dithering to the coin
  vec3 color = mix(u_colorBack.rgb, texColor.rgb, res);
...
export class DitheringEffectSimplex {
  constructor(container, imageSrc, width, height) {
    ...
    // Coin parameters
    this.coinSize = 2.0 // Size of the coin relative to screen width
  ...
  async init() {
    ...
    this.coinTexture = await new Promise((resolve, reject) => {
      textureLoader.load('/coin.png', resolve, undefined, reject)
    })
    
    // Create material for coin with dithering effect
    this.coinMaterial = new THREE.ShaderMaterial({
      vertexShader,
      ...
      uniforms: {
        ...
        u_resolution: { value: new THREE.Vector2(this.width, this.height) },
        u_pixelRatio: { value: Math.min(window.devicePixelRatio, 2) },
        u_texture: { value: this.coinTexture },
        u_pxSize: { value: 6 },
        u_colorBack: { value: new THREE.Vector4(0.06, 0.06, 0.06, 1.0) },
        u_colorFront: { value: new THREE.Vector4(1.0, 1.0, 1.0, 1.0) },
        u_shape: { value: DitheringShapes.simplex },
        u_type: { value: DitheringTypes['8x8'] }
      },
      transparent: true
    })
    
    // Create coin geometry and mesh
    const coinGeometry = new THREE.PlaneGeometry(this.coinSize, this.coinSize)
    this.coinMesh = new THREE.Mesh(coinGeometry, this.coinMaterial)
    
    // Position coin in right bottom corner
    this.coinMesh.position.x = 0.5 - this.coinSize / 2
    this.coinMesh.position.y = -0.5 + this.coinSize / 2
  ...
  animate() {
    ...
    // Update coin material time uniform
    if (this.coinMaterial) {
      this.coinMaterial.uniforms.u_time.value = this.time
    }
