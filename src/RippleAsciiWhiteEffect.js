...
// Fragment shader - ripple click triggers white ASCII scramble effect over clean SVG
const fragmentShader = `
  ...
  void main() {
    ...
    // In ripple: show pure white letters with transparency
    vec3 finalColor = mix(originalColor.rgb, vec3(1.0), totalRipple * letterVisible);
    
    // Alpha channel - transparent where no letters, opaque where letters appear
    float finalAlpha = mix(originalColor.a, max(originalColor.a, letterVisible), totalRipple);
    
    gl_FragColor = vec4(finalColor, finalAlpha);
  }
`

export class RippleAsciiWhiteEffect {
